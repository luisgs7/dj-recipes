...
class RecipeAPIv2TestMixin(RecipeMixin):
    ...
    def get_auth_data(self,
        ...
        response = self.client.post(
            reverse('recipes:token_obtain_pair'), data={
                **userdata,
            }
        )
        return {
...
class RecipeAPIv2Test(test.APITestCase, RecipeAPIv2TestMixin):
    ...
    @patch('recipes.views.api_cbv_v2.RecipeAPIv2Pagination.page_size', new=7)
    ...
    @patch('recipes.views.api_cbv_v2.RecipeAPIv2Pagination.page_size', new=10)
    def test_recipe_api_list_loads_recipes_by_category_id(self):
        ...
        respose = self.get_recipe_api_list(reverse_result=api_url)

        # We should only see recipes from the wanted category
        self.assertEqual(
            len(respose.data.get('results')),
            9
        )
